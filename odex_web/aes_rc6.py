...
"""
from .RC6.Enc_Dec import rc6_encrypt, rc6_decrypt
...
class AES:
    ...
    def encrypt_block(self, plaintext, rc6_enabled = False, trace_enabled = False):
        """
        ...
        if trace_enabled: trace += ("\n=========== inital =========== for {}".format(matrix2bytes(plain_state).hex()))
        ...
        for i in range(1, self.n_rounds):
            ...
            if trace_enabled: trace += "\n" +  ("Sr -> {}".format( matrix2bytes(plain_state).hex() ) )
            mix_columns(plain_state)
            if trace_enabled: trace += "\n" +  ("Mc -> {}".format( matrix2bytes(plain_state).hex() ) )
            add_round_key(plain_state, self._key_matrices[i])
            if trace_enabled: trace += "\n" +  ("Rk -> {}".format( matrix2bytes(plain_state).hex() ) )
			
            if (rc6_enabled and (i == 3 or i == 6) ):
                data_hex = matrix2bytes(plain_state).hex().upper()
        ...
        return [matrix2bytes(plain_state), trace]

    def decrypt_block(self, ciphertext, rc6_enabled = False, trace_enabled = False):
        """
        ...
        if trace_enabled: trace += "\n" +  ("Sr -> {}".format( matrix2bytes(cipher_state).hex() ) )
        ...
        for i in range(self.n_rounds - 1, 0, -1):
            ...
            if trace_enabled: trace += "\n" +  ("Mc -> {}".format( matrix2bytes(cipher_state).hex() ) )
            inv_shift_rows(cipher_state)
            if trace_enabled: trace += "\n" +  ("Sr -> {}".format( matrix2bytes(cipher_state).hex() ) )
            inv_sub_bytes(cipher_state)
            if trace_enabled: trace += "\n" +  ("Sb -> {}".format( matrix2bytes(cipher_state).hex() ) )
            
            if (rc6_enabled and (i == 4 or i == 7 ) ):
                data_hex = matrix2bytes(cipher_state).hex().upper()
                rc6_text = bytes.fromhex( rc6_decrypt(data_hex, self._key.hex().upper() ) )
        ...
        return [matrix2bytes(cipher_state), trace]
